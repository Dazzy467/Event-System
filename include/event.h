...
public:
    mouseEvent(int id = -1, int xi = 0, int yi = 0) : x(xi), y(yi) {
        this->id = id;
    }
...
{
public:
    mouseEventListener(eventSystem& eventSystemObj)
    {
        eventSystemObj.Bind<mouseEvent>(EventType::EVENT_MOUSE_ANY,std::bind(&mouseEventListener::OnEvent,this,std::placeholders::_1));
    }

    void OnEvent(const mouseEvent& e)
    {
        std::cout << "Triggered mouse event id: " << e.getId() << std::endl;
    }
};